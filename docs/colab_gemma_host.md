...
Shared inference for Heinzy: **Gemma 2 12B via Ollama on Google Colab**, exposed
...
3. Colab Secrets → add `NGROK_AUTHTOKEN` (ngrok dashboard → Your Authtoken).
4. Runtime → Run all.
5. Confirm the self-check cell prints `gemma2:12b` in `/api/tags` and a chat reply.
...
# Sanity (should list gemma2:12b):
curl -sS "$MODEL_ENDPOINT/api/tags"
```

Align `config.yaml` → `model.tag: gemma2:12b` (repo default after #16), or
override for one shell:

```bash
MODEL_TAG=gemma2:12b python scripts/ask_handbook.py --query "how many electives?"
```
